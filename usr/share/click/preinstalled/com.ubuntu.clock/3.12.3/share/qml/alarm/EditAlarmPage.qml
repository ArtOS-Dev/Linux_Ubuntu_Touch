...
Page {
    ...
    function updateAlarm() {
        ...
        alarmTime.setHours(_timePicker.hours, _timePicker.minutes,  (tempAlarm.date.getTime() % 60000 ? 0 : 1)) 
    ...
    */
    ...
    Column {
        ...
        ListItem {
            ...
            readonly property string defaultAlarmSound: clockAppSettings.defaultAlarmSound ?
                                                            alarmSoundHelper.getSoundName(clockAppSettings.defaultAlarmSound.toString()) :
                                                            "Alarm clock"
            readonly property string defaultAlarmSoundFileName: clockAppSettings.defaultAlarmSound ?
                                                                    clockAppSettings.defaultAlarmSound.toString().replace(/.*\//,'') :
                                                                    "Alarm clock.ogg"
            ...
            onClicked: {
                pageStack.push(Qt.resolvedUrl("AlarmSound.qml"), {
                                    "alarmSoundName": _alarmSound.subText,
                                    "alarmUrl": _alarm.sound,
                                    "defaultAlarmSound" : _alarmSound.defaultAlarmSound
                                });
                pageStack.currentPage.setAlarm.connect(setAlarm);
            }

            function setAlarm(newAlarmUrl) {
                _alarm.sound = newAlarmUrl;
                _alarmSound.subText = alarmSoundHelper.getSoundName(newAlarmUrl);
            }
        }
    }

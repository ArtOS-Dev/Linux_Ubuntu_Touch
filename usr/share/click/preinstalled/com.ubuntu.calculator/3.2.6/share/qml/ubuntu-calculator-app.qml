...
MainView {
    ...
    // Used to determine which keyboard to use
    property bool isLandscape: scrollableView.width > scrollableView.height
    ...
    PageStack {
        ...
        Page {
                ...
                Screen {
                    ...
                    trailingActions: ListItemActions {
                        actions: [
                            ...
                            Action {
                                id: screenDelegateEditResultAction
                                iconName: "edit-paste"
                                text: i18n.tr("Edit Result")
                                onTriggered: {
                                    longFormula = model.result;
                                    shortFormula =  model.result;
            ...
            ScrollableView {
                anchors {
                    fill: parent
                    bottomMargin: bottomEdge.enabled
                        ? bottomEdge.hint.height
                        : units.gu(1)
                    topMargin: mainHeader.visible
                        ? mainHeader.height
                        : 0
                }
                ...
                Rectangle {
                    ...
                    TextField {
                        ...
                        function keyPress(event) {
                            if (!(event.modifiers & Qt.ControlModifier || event.modifiers & Qt.AltModifier)) { // Shift needs to be passed through as it may be required for some special keys
                                ...
                                if (bottomEdge.contentItem) {
                                    bottomEdge.contentItem.pressedKey = event.key;
                                    bottomEdge.contentItem.pressedKeyText = event.text;
                                }
                        ...
                        function keyRelease(event) {
                            ...
                            if (bottomEdge.contentItem) {
                                bottomEdge.contentItem.pressedKey = -1;
                                bottomEdge.contentItem.pressedKeyText = "";
                            }
                        }

                        //TODO: Find a better way of doing this?
                        //Don't let keyboard to show
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            textInputField.cursorPosition = textInputField.positionAt(mouseX,TextInput.CursorOnCharacter)
                        }
                        onPressAndHold: {
                            textInputField.cursorPosition = textInputField.positionAt(mouseX,TextInput.CursorOnCharacter)
                            PopupUtils.open(Qt.resolvedUrl("ui/TextInputPopover.qml"), textInputField, {target: textInputField})
                        }
                    }
                }
                ...
                Loader {
                    ...
                    source: isLandscape ? "ui/LandscapeKeyboard.qml" : "ui/PortraitKeyboard.qml"
            ...
            BottomEdge {
                ...
                height: keyboardLoader.height + hint.height - units.gu(1)

                onCollapseCompleted: {
                    // FIXME this is a workaround that fixes a space below the
                    // keyboard after the bottom edge is collapsed and the screen
                    // is rotated
                    scrollableView.contentY = 0;
                    scrollableView.scrollToBottom();
                }

                // TODO tap outside the bottom edge to close it

                contentUrl: Qt.resolvedUrl("ui/BottomEdgePage.qml")
                enabled: textInputField.visible && !isLandscape && mathJs != null
                hint.text: i18n.tr("Functions")
